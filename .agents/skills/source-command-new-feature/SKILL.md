...
1. **Refuse without a plan.** If `.Codex/plans/YYYY-MM-DD-<name>.md` does not exist, write one first. The plan must include: goal, regulatory anchor, schema fields touched, eval impact, test cases.
...
- `.Codex/skills/write-test-first.md`
- `.Codex/skills/add-validation-rule.md` (if validation rule)
- `.Codex/skills/add-stress-scenario.md` (if stress scenario)
- `.Codex/skills/add-sicr-trigger.md` (if SICR trigger)
- `.Codex/skills/add-anomaly-detector.md` (if anomaly detector)
